...
def _get_candles_moex(ticker, days=HISTORY_DAYS, interval=24):
    ...
    # [FIX] Индексы (IMOEX, RTSI, и т.п.) лежат в market=index / board=SNDX,
    # а не в shares/TQBR. Без этого get_candles("IMOEX") всегда пустой.
    if ticker.upper() in ("IMOEX", "RTSI", "IMOEX2", "MOEXBC", "MOEXBMI", "MOEXFN",
                          "MOEXOG", "MOEXCH", "MOEXMM", "MOEXEU", "MOEXTL",
                          "MOEXTN", "MOEXCN", "MOEXIT", "MOEXREIT"):
        url = (f"{MOEX_ISS_URL}/engines/stock/markets/index"
...
def get_orderbook(ticker: str, depth: int = 20) -> dict:
    """
    Возвращает агрегированные уровни поддержки / сопротивления из стакана
    через MOEX ISS: /engines/stock/markets/shares/boards/TQBR/securities/{ticker}/orderbook.json

    Алгоритм:
      • Берём top-N заявок bid (покупка) и ask (продажа)
      • Поддержка  = уровень bid с максимальным объёмом (в деньгах = price × qty)
      • Сопротивление = уровень ask с максимальным объёмом

    Возвращает: {"support": float|None, "resistance": float|None}
    [N]
    """
    # [FIX] Корректный ISS-эндпоинт для стакана: без /boards/TQBR/ в пути,
    # доска передаётся в query как securityboard=TQBR. Старый URL отдавал 404 HTML.
    url = (f"{MOEX_ISS_URL}/engines/stock/markets/shares"
           f"/securities/{ticker}/orderbook.json")
    params = {"iss.meta": "off", "depth": depth, "securityboard": "TQBR"}
    try:
        resp = _get_with_retry(url, params)
        # [FIX] Защита от пустого ответа / HTML-страницы — не валимся в JSONDecodeError
        txt = (resp.text or "").strip() if hasattr(resp, "text") else ""
        if not txt or txt[0] not in "{[":
            # вне сессии ISS может отдавать пусто — это нормально, не warning
            logger.debug(f"[N] Стакан {ticker}: пустой/не-JSON ответ")
            return {"support": None, "resistance": None}
        data = resp.json()
    except Exception as e:
        logger.warning(f"[N] Стакан {ticker}: {e}")
        return {"support": None, "resistance": None}
    ...
    try:
        ...
        price_i = col_idx.get("PRICE")
        qty_i   = col_idx.get("QUANTITY")
        side_i  = col_idx.get("BUYSELL")
        if price_i is None or qty_i is None or side_i is None:
            return {"support": None, "resistance": None}
        ...
        for row in rows:
            ...
            if price is None or qty is None:
                continue
            volume_money = float(price) * float(qty)
            if side == "B":
                bids.append((float(price), volume_money))
            elif side == "S":
                asks.append((float(price), volume_money))

        support    = max(bids, key=lambda x: x[1])[0] if bids else None
        resistance = max(asks, key=lambda x: x[1])[0] if asks else None
        return {"support": support, "resistance": resistance}

    except Exception as e:
        logger.warning(f"[N] Парсинг стакана {ticker}: {e}")
        return {"support": None, "resistance": None}
