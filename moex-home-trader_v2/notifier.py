...
# [FIX] Утилиты для безопасной отправки: html.escape и нарезка сообщений.
# Используй _esc(x) для любых ВНЕШНИХ строк (заголовки новостей, тикеры из API),
# и _split_msg(text) перед отправкой больших сводок (> 3900 символов).
import html as _html_fix

def _esc(x):
    """HTML-escape для Telegram parse_mode=HTML."""
    return _html_fix.escape("" if x is None else str(x), quote=False)

def _split_msg(text: str, limit: int = 3900):
    """Режет длинный текст на части по границам строк, чтобы не превысить лимит Telegram."""
    if len(text) <= limit:
        return [text]
    parts, buf, size = [], [], 0
