...
def analyze_ema_trend(df: pd.DataFrame) -> dict:
    ...
    if death:
        return {"score": -2, "label": f"EMA50/200: мёртвый крест",
                "ema50": round(c50,2), "ema200": round(c200,2)}
    if c50 > c200:
        return {"score":  0, "label": f"EMA50 > EMA200 — тренд вверх",  # [BT] acc 47% → нейтрализован
                "ema50": round(c50,2), "ema200": round(c200,2)}
    return {"score":  0, "label": f"EMA50 < EMA200 — тренд вниз",  # [BT2] acc 43.8% → нейтрал
            "ema50": round(c50,2), "ema200": round(c200,2)}
...
def analyze_obv(df: pd.DataFrame) -> dict:
    ...
    if not (r_cl > p_cl) and (r_obv > p_obv):
        return {"score":  0, "label": "OBV: бычья дивергенция (нейтрал)",
                "obv_trend": "растёт"}  # [BT] acc 43% → нейтрализован
    if (r_cl > p_cl) and not (r_obv > p_obv):
        return {"score":  0, "label": "OBV: медвежья дивергенция (нейтрал)",  # [BT2] acc 44.0% → нейтрал
                "obv_trend": "падает"}
...
def analyze_rsi_divergence(df: pd.DataFrame, rsi: pd.Series, lookback: int = 14) -> dict:
    ...
    if cmnp < pmnp and cmnr > pmnr:
        return {"score":  3, "label": f"RSI дивергенция бычья"}  # [BT] acc 57.5% → вес +3
...
def analyze_ticker(df: pd.DataFrame, current_price: float,
    ...
    # RSI
    if not pd.isna(cr):
        if   cr < 30: ts += 3; reasons.append(f"RSI {cr:.0f} — перепроданность")  # [BT2] acc 75.7% → +3
        elif cr < 40: ts += 2; reasons.append(f"RSI {cr:.0f} — зона покупки")         # [BT2] acc 75.7% → +2
        elif cr > 70: ts -= 1; reasons.append(f"RSI {cr:.0f} — перекупленность")  # [BT2] acc 51.7% → ослаблен
        elif cr > 60: pass                                                              # [BT2] acc 51.7% → убран

    # MACD
    if   cm > cs and ph < 0 and ch > 0: ts += 2; reasons.append("MACD: бычье пересечение")
    elif cm > cs:                        ts += 1; reasons.append("MACD выше сигнальной")
    elif cm < cs and ph > 0 and ch < 0: ts -= 1; reasons.append("MACD: медвежье пересечение")  # [BT2] acc 46.2% → -1
    elif cm < cs:                        pass                                                    # [BT2] просто ниже → убран
    ...
    # Боллинджер
    if not pd.isna(cbp):
        if   cbp < 0:   ts += 1; reasons.append(f"Боллинджер: ниже нижней полосы")   # [BT2] acc 63.6% → +1 восстановлен
        elif cbp < 0.2: pass  # [BT2] у нижней — слишком слабо, нейтрал
        elif cbp > 1:   ts -= 1; reasons.append(f"Боллинджер: выше верхней полосы")   # [BT3] acc 16.7% на 10д → -1
        elif cbp > 0.8: pass  # [BT3] acc 16.7% медвежий — убран

    # Объём
    if   vol["signal"] == "бычий объём":   ts += 1; reasons.append(f"Аномальный объём при росте (x{vol['ratio']})")
    # elif vol["signal"] == "медвежий объём": pass  # [BT2] acc 45.0% → убран

    # MA выравнивание
    if   last > cm5 > cm10 > cm20: pass  # [BT] MA_align бычий acc 43% → убран
    elif last < cm5 < cm10 < cm20: pass  # [BT2] MA_align медвежий acc 37.8% → убран
