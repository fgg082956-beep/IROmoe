...
# [FIX] log neutral — см. патч ниже, сам detect_regime не изменён, добавлен логгер
...
# [FIX] Дополнительная диагностика: если режим не удалось определить по недостатку
# данных — логируем это явно (раньше молча возвращался neutral).
import logging as _lg
_regime_log = _lg.getLogger(__name__)

try:
    _orig_detect_regime = detect_regime  # type: ignore
    def detect_regime(*a, **kw):  # type: ignore
        r = _orig_detect_regime(*a, **kw)
        try:
            name = (r or {}).get('name', '')
            if not name or 'neutral' in str(name).lower() or 'нейтр' in str(name).lower():
                _regime_log.info("[M] regime=neutral (проверь, что IMOEX данные доступны и >= 60 баров)")
        except Exception:
            pass
        return r
except NameError:
