...
import tempfile as _tf_fix
import threading as _th_fix
_NS_LOCK = _th_fix.Lock()
...
def save_data(data: dict) -> None:
    # [FIX] atomic write под Lock
        with _NS_LOCK:
            _dir = _os_fix.path.dirname(_os_fix.path.abspath(DATA_FILE)) or "."
            _fd, _tmp = _tf_fix.mkstemp(prefix=".news_", suffix=".json", dir=_dir)
            try:
                with _os_fix.fdopen(_fd, "w", encoding="utf-8") as _fh:
                    json.dump(data, _fh, ensure_ascii=False, indent=2)
                _os_fix.replace(_tmp, DATA_FILE)
            except Exception:
                if _os_fix.path.exists(_tmp): _os_fix.remove(_tmp)
                raise
